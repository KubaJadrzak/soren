    ...
    module Options
      class SpecificTimeoutsTest < Minitest::Test
        def test_read_timeout_uses_default
          ...
          assert_equal Soren::Defaults::Options::READ_TIMEOUT, timeout.to_f
        end

        def test_connect_timeout_uses_default
          timeout = Timeout::ConnectTimeout.new(nil)

          assert_equal Soren::Defaults::Options::CONNECT_TIMEOUT, timeout.to_f
        end

        def test_write_timeout_uses_default
          timeout = Timeout::WriteTimeout.new(nil)

          assert_equal Soren::Defaults::Options::WRITE_TIMEOUT, timeout.to_f
        end

        def test_specific_timeout_accepts_explicit_value
          timeout = Timeout::ReadTimeout.new('1.2')

          assert_equal 1.2, timeout.to_f
        end
      end
    end
  end
